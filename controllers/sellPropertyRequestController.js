...
const handleRequestToBuyProperty = async (req, res) => {
    try {
        ...
        // Get buyer details from the authenticated user
        ...
        // Check if buyer already has a pending request
        const existingRequest = await SellRequest.findOne({
            property: propertyId,
            buyer: buyerId,
...
// Get buyer's sell requests
const handleGetBuyerRequests = async (req, res) => {
    try {
        const buyerId = req.user._id;
        const requests = await SellRequest.find({ buyer: buyerId })
            .populate('property', 'title price address city media images')
            .sort({ createdAt: -1 });

        res.status(200).json({
            success: true,
            message: "Sell requests retrieved",
...
const handleUploadPaymentReceipt = async (req, res) => {
    try {
        ...
        // Check if buyer owns this request
        if (sellRequest.buyer.toString() !== req.user._id.toString()) {
        ...
        if (sellRequest.paymentDetails && sellRequest.paymentDetails.receiptImage) {
            return res.status(400).json({ 
                success: false, 
        ...
        // Update deal details
        sellRequest.dealDetails = {
            amount: amount || sellRequest.property.price,
        ...
        const history = new History({
            ...
            user: req.user._id,
            propertyId: sellRequest.property._id,
        ...
        res.status(200).json({
            ...
            request: {
                ...
                dealDetails: {
                    amount: sellRequest.dealDetails.paymentSchedule.amount,
                    method: sellRequest.dealDetails.paymentSchedule.paymentMethod,
                    reference: sellRequest.dealDetails.paymentSchedule.referenceNumber,
                    paymentDate: sellRequest.dealDetails.paymentSchedule.paymentDate,
                    verified: sellRequest.dealDetails.paymentSchedule.verified
                }
            }
        });
